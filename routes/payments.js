...
router.post("/mercadopago/create-preference", (req, res) => {
  try {
    ...
    getMercadoPagoConfig(environment || "production", (err, config) => {
      if (err)
        return handleQueryError(err, res, "obtener configuración MercadoPago");
...
router.post("/paypal/create-order", (req, res) => {
  try {
    ...
    getPayPalConfig(environment, (err, config) => {
      if (err)
        return handleQueryError(err, res, "obtener configuración PayPal");
      ...
      axios
        ...
        .then((tokenResponse) => {
          ...
          axios
            .post(`${config.baseUrl}/v2/checkout/orders`, orderData, {
              headers: {
                Authorization: `Bearer ${accessToken}`,
                "Content-Type": "application/json",
              },
            ...
            .then((orderResponse) => {
              ...
              res.json({
                ...
                original_amount_mxn: amount,
                approve_url: orderResponse.data.links?.find(
    ...
    getPayPalConfig(environment, (err, config) => {
      if (err)
        return handleQueryError(
          err,
          res,
          "obtener configuración PayPal para captura"
        );
      ...
      axios
        ...
        .catch((tokenError) => {
          console.error(
            "❌ Error obteniendo token PayPal para captura:",
            tokenError
          );
          res.status(500).json({
            error: "Error de autenticación PayPal",
      ...
      getMercadoPagoConfig(environment || "production", (err, config) => {
        if (err)
          return handleQueryError(
            err,
            res,
            "obtener configuración MercadoPago para status"
          );
      ...
      getPayPalConfig(environment, (err, config) => {
        if (err)
          return handleQueryError(
            err,
            res,
            "obtener configuración PayPal para status"
          );
        ...
        axios
          ...
          .then((tokenResponse) => {
            ...
            axios
              .get(`${config.baseUrl}/v2/checkout/orders/${paymentId}`, {
                headers: {
                  Authorization: `Bearer ${accessToken}`,
                },
                timeout: 10000,
          ...
          .catch((error) => {
            console.error(
              "❌ Error obteniendo token PayPal para status:",
              error
            );
            res.status(500).json({
              error: "Error de autenticación PayPal",
