  ...
  try {
    const sql = `
      SELECT id, nombre, aPaterno, aMaterno, email, puesto, estado, 
             fecha_creacion, ultima_actualizacion, imagen
      FROM empleados
      ORDER BY nombre ASC;
    `;
...
router.post('/', async (req, res) => {
  try {
    const { nombre, aPaterno, aMaterno, email, password, puesto, estado, imagen } = req.body;
    ...
    db.query(checkEmailSql, [email], async (err, result) => {
      ...
      async function procedeToSaveEmployee() {
        try {
          ...
          const insertSql = `
            INSERT INTO empleados (nombre, aPaterno, aMaterno, email, password, puesto, estado, imagen, fecha_creacion, ultima_actualizacion)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
          `;
          
          db.query(
            insertSql, 
            [nombre, aPaterno, aMaterno || null, email, hashedPassword, puesto, estado || 'activo', imagen || null, currentDate, currentDate],
            (err, result) => {
              if (err) {
    ...
    db.query(checkEmailSql, [email, id], async (err, result) => {
      ...
      async function proceedToUpdateEmployee() {
        try {
          ...
          // Campos básicos
          updateFields.push('nombre = ?', 'aPaterno = ?', 'aMaterno = ?', 'email = ?', 'puesto = ?', 'estado = ?', 'ultima_actualizacion = ?');
          updateValues.push(nombre, aPaterno, aMaterno || null, email, puesto, estado, currentDate);
